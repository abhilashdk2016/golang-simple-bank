...
const addUpdateAccount = `-- name: AddUpdateAccount :exec
...
func (q *Queries) AddUpdateAccount(ctx context.Context, arg AddUpdateAccountParams) error {
	_, err := q.db.ExecContext(ctx, addUpdateAccount, arg.Amount, arg.ID)
	return err
}
...
UPDATE accounts SET balance = $3 WHERE id = $1 AND owner = $2 RETURNING id, owner, balance, currency, created_at
`
